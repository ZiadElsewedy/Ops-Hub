...
| **Tests** | **1482 pass · 0 fail** across 202 files (~36s) — **green**; the two long-standing splash-centering failures were fixed 2026-08-03, and `manager_home_test.dart` (10) was added with the Manager Home rebuild the same day. Cloud Functions: **83 pass** (`cd functions && node --test`); **Firestore rules: 61 pass** (`cd firestore-tests && npm test` — needs the Firebase CLI + a JDK). All three re-run and verified **2026-08-03**. NestJS chat backend: **84 pass** (`cd ~/Desktop/Developer/drop-api && npx jest`) — separate repo, last verified 2026-08-02 |
| **Tests** | **1487 pass · 0 fail** across 203 files (~36s) — **green**, re-run and verified **2026-08-03** after the chat stability audit. Cloud Functions: **83 pass** (`cd functions && node --test`); **Firestore rules: 61 pass** (`cd firestore-tests && npm test` — needs the Firebase CLI + a JDK), both verified the same day. NestJS chat backend: **105 pass** (`cd ~/Desktop/Developer/drop-api && npx jest`) — separate repo, verified 2026-08-03 |
...
(`drop-api` **92 pass**, app **1487 pass**) but the **server is not deployed**,
...
**`flutter test` is fully green for the first time in weeks: 1487 pass, 0 fail.**
...
flutter test                             # expect: 1487 pass, 0 fail — GREEN; any red is a real regression
