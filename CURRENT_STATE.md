...
| **Tests** | **1504 pass · 0 fail** across 203 files (~35s) — **green**; the two long-standing splash-centering failures were fixed 2026-08-03, and `manager_home_test.dart` (10) and `today_roster_test.dart` (7) were added with the Manager Home rebuild the same day. Cloud Functions: **83 pass** (`cd functions && node --test`); **Firestore rules: 61 pass** (`cd firestore-tests && npm test` — needs the Firebase CLI + a JDK). All three re-run and verified **2026-08-03**. NestJS chat backend: **84 pass** (`cd ~/Desktop/Developer/drop-api && npx jest`) — separate repo, last verified 2026-08-02 |
