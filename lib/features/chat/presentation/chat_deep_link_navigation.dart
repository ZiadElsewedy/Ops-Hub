...
void openChatDeepLink(
  ...
  router.whenReady(() {
    ...
    // whatever the last `go` set. Guarding on it would compare against a stale
    // location and let a re-tap stack a duplicate thread.
    if (router.currentLocationOrNull == destination) return;
    ...
    // This runs only after startup has restored the session, so the router
