...
> [ATTENDANCE_PRODUCT_REDESIGN_PLAN.md](ATTENDANCE_PRODUCT_REDESIGN_PLAN.md) §7,
> after a real store manager could not read the surface this subsection
> specified. §6.4–§6.10 are replaced in full. Everything above (§6.1–§6.3 — week
> definition, audience, close inputs) is unchanged and still binds, as does
> [ADR-017](../decisions/ADR-017-attendance-reporting-ledger.md)'s ledger scope
> and metric bar.
>
> **Why the spec had to change, not just the screen.** The shipped report was a
> faithful build of the previous §6.4/§6.5: eight sections and six metrics,
> exactly as written. Fixing only the screen would have regenerated them from
> this document. The eight sections are not deleted — most move to the audience
> that has a decision to make with them.

**Five sections. Fixed order.**

| # | Section | Content | Decision it drives | Renders |
| --- | --- | --- | --- | --- |
| 1 | Needs your attention | Unresolved items with an inline route to resolve them | What must I settle before this week is done? | **Only when non-empty** |
| 2 | The week in one line | Branch · dates · shifts worked of scheduled · hours · overtime | Can I state this week to my district manager? | Always |
| 3 | By person | Per-employee facts, exceptions first | Who do I coach or verify? | Always |
| 4 | By day | Seven days, scheduled vs worked | Which day of the week keeps failing? | Always |
| 5 | Share | PDF summary · timesheet spreadsheet | Hand this up or reconcile it | Always |

Sections 1–3 answer the manager's whole job. Section 1 is first because it is the
only one with a verb; in a week where Daily Close and the Exception Queue (§8.1,
§10) have done their work it does not render at all, and its absence is itself
information.

**Where the removed sections went.**

| Removed from Weekly | New home | Why |
| --- | --- | --- |
| Close readiness | Daily Close (§8.1) for the operational half; Admin Workspace for lock/version | It conflated *are my exceptions clear* with *is this period locked* — two audiences |
| Metric strip (as specified) | Replaced by §6.5 below | Six metrics, four of which no manager is accountable for |
| Exception summary | Exception Queue (§10) | Exceptions belong where they are resolved, not where they are counted |
| Employee rows | Kept, as **By person**, re-sorted per §6.5.1 | The table was right; its order was not |
| Evidence table | Admin Workspace; managers get the per-employee drill-down instead | Row-level evidence is an audit need |
| Export/restatement panel | Split: manager share (§6.10) and admin payroll/restatement | *Restatement* is an accounting term for correcting published statements |

### 6.5 Metrics

**Four. Not six.** Each names formula, denominator, owner, and — per ADR-017's
metric bar — the decision it changes. A metric that cannot name the decision does
not ship, however cheap it is to compute.

| Metric | Formula | Denominator | Owner | Decision |
| --- | --- | --- | --- | --- |
| Hours worked | sum `workedMinutes` | Present rows | Calculator → rollup | Labour cost; the number the manager is accountable for |
| Overtime | sum `overtimeMinutes` past grace | Completed rows with a scheduled end | Calculator → rollup | Cost control and authorisation |
| Unexcused absences | count unexcused no-show rows | Expected work shifts | Rollup | Coverage intervention; who to talk to |
| Late arrivals | **count** of arrivals past grace | Present scheduled arrivals | Calculator → rollup | Coaching pattern |

**Removed, with reasons.**

| Removed | Reason |
| --- | --- |
| **Show-up rate** | A **deliberate reversal** of the previous §6.5, which made it the headline. At one expected shift, `0%` is statistically meaningless and emotionally alarming — the exact failure the manager hit. Percentages need volume. **It survives at admin / multi-branch level**, where a denominator exists, and on the hub headline. It is not a store-level headline. |
| **Punctual arrival rate** | Rendered `--` in the observed week. Duplicates late-arrival count with worse behaviour at low volume. |
| **Late minutes (summed)** | Nobody manages to this number. "Three late arrivals" is actionable; "47 aggregate minutes" is not. Minutes stay in the per-person table where they explain a specific person. |
| **Exception count** | An internal integrity metric wearing a KPI costume. Once §8.1/§10 exist it is structurally near zero, and a metric that is always zero teaches managers to stop looking. |

Late, early-leave, and overtime remain derived facts, not persisted statuses
(`docs/design/ATTENDANCE_SPEC.md:371`,
`lib/core/enums/attendance_status_filter.dart:3`).

#### 6.5.1 Sorting

**By person: exceptions first, then alphabetical within each band.**
...
not judging a person, and the report still computes no score, no rank, and no
composite.

**By day: chronological, always seven rows,** Sunday→Saturday `Africa/Cairo`.
...
| 2 shifts need a decision                                [Review these]   |
+--------------------------------------------------------------------------+
...
+----------------+----------------+----------------+----------------------+
| Hours worked   | Overtime       | Unexcused      | Late arrivals        |
| 312h           | 4h             | 3              | 7                    |
+----------------+----------------+----------------+----------------------+

+--------------------------------------------------------------------------+
| By person                          (exceptions first, then alphabetical) |
| Employee    Scheduled Worked Absent Late min Hours Overtime  Status      |
| Omar        5         4      1      50       31h   0         No-show     |
| Dina        5         5      0      12       39h   0         -           |
+--------------------------------------------------------------------------+

+--------------------------------------------------------------------------+
| By day                                                                    |
| Sunday 26 Jul     No data                                                |
| Monday 27 Jul     8 shifts   8 scheduled   8 worked   0 absent           |
+--------------------------------------------------------------------------+

+--------------------------------------------------------------------------+
| Share this week                        [PDF]  [Spreadsheet]              |
+--------------------------------------------------------------------------+
```
...
4 metric cards (1 column)
By person   — cards, exceptions first, tap -> employee
By day      — seven rows
Share this week
```

The previous "Summary | Exceptions | Employees | Rows" segmented control is
retired with the sections it navigated. Five sections do not need a nav control.

### 6.8 Drill-downs

| Drill-down | Opens | Reason |
| --- | --- | --- |
| Needs-your-attention row | Exception Queue filtered to week/branch/type | Work the decision |
| Person row | Per-Employee Report scoped to the week | Explain a person's record |
| Day row | Daily Close for that date | Resolve that day |

The **shift-row drill-down is removed from the manager surface** with the evidence
table. Row-level evidence remains reachable in Admin Workspace and from the
per-employee report; a manager who needs one record gets there through the
person, which is how they think about it.

### 6.9 States

**Manager-facing status is `AttendanceCoverageStatus`** — one vocabulary, shared
with Monthly, implemented at
`lib/features/attendance/domain/reporting/attendance_coverage_status.dart`.

| Manager sees | Meaning | Pipeline state |
| --- | --- | --- |
| **No data yet** | No rows at all. No rates, no percentages, no export. | awaiting close |
| **Needs attention** | Rows exist; something needs a decision. **The only toned status.** | partially closed |
| **In progress** | Rows exist, nothing blocked, some dates uncovered | "fully closed" with day gaps |
| **Settled** | Every date covered, nothing blocked | fully closed |

`open → ready → locked → exported → restated` still exists and is still what the
close pipeline computes. It is an **admin** vocabulary; a manager never sees
*locked*, *exported*, or *restated*, and the Phase 3 Admin Workspace owns them.

**Empty states are three different situations and must never render alike:**

| Situation | Copy | Tone |
| --- | --- | --- |
| No rows for a date | "No data" | Tertiary grey — an unknown, not a result |
| Date has rows, nobody worked | `0%` / `0 of N worked` | Error red — a real result |
| Whole period has no rows | "No data yet" | Neutral — no rates rendered |

> **Known limit, and the Phase 1 boundary.** The report reads materialised ledger
> rows only, so it **cannot distinguish "nobody was scheduled" from "scheduled but
> never captured"** — both are honestly *No data*. Splitting them needs the roster
> in the read path, which
> `test/attendance_reporting_source_guard_test.dart` forbids for these files and
> which ADR-017 keeps out of the denominator on purpose. **It therefore needs its
> own decision, not a UI change.** Until then the surface says *No data* and
> never presents a gap as a result.

### 6.10 Exports

| Export | Audience | Available when |
| --- | --- | --- |
| PDF summary | Manager, Admin | Any **settled** week — lock is not required |
| Timesheet spreadsheet | Manager, Admin | Any settled week |
| Payroll export | **Admin only** | **Locked periods only** — never on this surface |

This amends the previous rule that Weekly offered PDF and CSV *only when locked*.
Managers need to share a week before payroll finalises it, and neither artefact
carries financial authority. **Payroll export keeps the lock gate and moves to
Admin Workspace**, so it never sits one tap from a PDF button.

PDF is for reading: the five sections above, plus footer. The spreadsheet is the
timesheet detail managers already reconcile in — one row per shift, schema per
section 12. Both are generated server-side (ADR-005); a client-authored payroll
artefact is unauditable.
