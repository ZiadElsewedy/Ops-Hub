...
- **Mobile is a different surface.** The 1600px landscape sheet is illegible shrunk to a phone, so on `!context.isDesktop` (<1024) the screen shows **`widgets/final_schedule_mobile_view.dart` (`FinalScheduleMobileView`)** — a day-by-day card list (Morning/Night, people as chips, hours + leave + day-notes inline), read top-to-bottom, no zoom. **macOS is untouched** — still the landscape sheet, `FittedBox`-scaled.
