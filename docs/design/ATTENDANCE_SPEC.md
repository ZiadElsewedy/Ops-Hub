...
| R11 | Managers create attendance directly? | **Yes — manager manual entry and direct resolve apply immediately with audit.** Managers are the branch authority **over their employees only**. A manager corrects/reviews an **employee's** attendance, never a **peer manager's** — only an **admin** corrects a manager (managers are peers; no manager-on-manager review, no self-review). Enforced server-side: the `attendance_corrections` create/update and the `attendance` reviewer-update rules require the target to be an `employee` when the actor is a manager (`isEmployeeUser`), admins unrestricted. The branch board, Daily Review, Corrections, and reports also **show a manager only their employees plus their own row** — a client-side visibility filter (a Firestore collection query can't be partially filtered by rules, so reads stay branch-scoped; this narrows what is shown, not what is technically readable). |
